...
func RunPoint4(ctx *helpers.TestContext, r *helpers.Report) {
	...
	if st, err := helpers.WaitForStatus(ctx, config.ReloadWaitTimeout, func(m map[string]helpers.ProcStatus) bool {
		p, ok := m["dummy:00"]
	...
	r.Section("TEST 4.2 — SIGHUP Reload Signal")
	...
	if _, err := helpers.RunCtl(ctx, "start lazy:00"); err != nil {
		...
		_, _ = helpers.WaitForStatus(ctx, config.DaemonReadyTimeout/3, func(m map[string]helpers.ProcStatus) bool {
			p, ok := m["lazy:00"]
	...
	r.Section("TEST 4.3 — Unchanged Programs Survive Reload")
	helpers.Must(helpers.CopyFile(ctx.BackupPath, ctx.ConfigPath))
	out, err = helpers.RunCtl(ctx, "status")
	helpers.Must(err)
	statusBefore = helpers.ParseStatus(out)
